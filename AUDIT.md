...
- [ ] **C10. `meter.fit()` doesn't change `meter.key` → stale cached parses.** `meter.py:122`. MED. ✓
...
- [ ] **C18. O(L×T) per-line scans + word_foot loop.** `vectorized.py:86,95`; `constraints.py:231`. LOW. `np_line == ln` per line inside loops → use sorted-group boundaries; `word_foot`'s per-line loop is a broadcastable outer product.
- [ ] **C19. `unres_within`/`unres_across` still Python loops** (known roadmap). `evaluate_constraints_batch`. Both reduce to a word-boundary mask `(L,N)` × same-position mask `(S,N)`.
...
- [ ] **T4. Partially consuming `parse_iter()` permanently truncates results.** `texts.py:280`. HIGH. ✓
...
- [ ] **T15. espeak discovery walks large trees at import + misses `libespeak-ng.so`.** `langs.py:494`. MED.
