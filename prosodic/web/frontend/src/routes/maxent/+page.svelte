<script>
	...
	function handleCorpusLoad(text, name) {
		corpusText = text;
		corpusName = name;
		// Clear file input since we're using corpus
	...
	function applyLearnedWeights(result) {
		...
		// Store zone weights for display on Meter page
		const zw = {};
...
<div class="page">
	...
	<p class="page-desc">Train constraint weights from a text corpus or annotated scansion data. Learned weights are saved to <a href="/meter">Meter</a> config.</p>
