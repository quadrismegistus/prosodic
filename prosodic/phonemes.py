...
@cache
def Phoneme(phon):
	# from gruut_ipa.features import to_vector, string_to_symbol, vector_to_features
	# phond = vector_to_features(to_vector(string_to_symbol(phon)))
	...
	if not phonl: 
		...
		if phon in get_ipa_info():
			...
			logger.error(f'No really what is this phoneme? {phon}')
			phond = {}
	else:
		phond = phonl[0].data
